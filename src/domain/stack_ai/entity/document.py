...
class Document(BaseModel):
    ...
    def validate_name(cls, value):
        if not value.strip():
            raise ValueError("[Argument Error] Document - name cannot be empty.")
        return value

    @field_validator("library_id")
    def validate_library_id(cls, value):
        if not value.strip():
            raise ValueError("[Argument Error] Document - library_id cannot be empty.")
        return value
    ...
    def register_index_config():  # @todo rethink this plugin approach: it will break if we change the config since it is only registered on table creation time.
        index_config = {
            ...
            "index_keys": ["library_id"],
        }
        # @todo add a BST index here for the created_at, but we need to refactor InMemoryDb.insert() to accept a list of indexers.
